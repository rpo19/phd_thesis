...
The second method, \textit{$\text{GNN}_\text{F}$}, also relies on GNN but uses a feature-based vectorizer following \textcite{shrimpton-etal-2015-sampling}. Mentions are encoded as character skip-bigram indicator vectors for surface text and TF-IDF vectors for context. For both $\text{GNN}_\text{B}$ and $\text{GNN}_\text{F}$.
...
Secondly, to understand how each component of the pipeline performs on its specific task, we also evaluate the modules separately. In particular, for the \ac{nel} module, we measure Recall@1\todo{dire che in el accruracy e recall@1 coicidono?}. For the NIL predictor, we compute precision, recall, and F1-score for the NIL class. Finally, for NIL clustering, following the co-reference evaluation methodology~\cite{streaming_crossdoc_coreference}, we calculate precision, recall, and F1 for MUC, B$^3$, and CEAF$_e$, also reporting the average F1 score.\todo{spiegare queste metriche? dove? in sota?}
...
% The NIL prediction component show high precision scores that grow from $b_0$ to $b_9$. Intuitively, this means that the number of false positives---non-NIL mentions predicted as NIL, in our case---is decreasing over the batches. 
...
The results of the most relevant experiments are reported in \Cref{tab:superresults}.
We can see that the performance of the \ac{nel} module (BLINK bi-encoder~\cite{blink}) is competitive compared to more complex models of state of the art: in particular, GENRE~\cite{decao2021autoregressive} obtains a R@1$_\text{total} = 74.7$ and R@1$_\text{unseen} = 70.4$---on mention-entity paris unseen at training-time---which are quite similar to ours (excluding error propagation).
...
The NIL prediction component, despite the high precision, suffers in recall, therefore yielding a high number of false negatives---or \emph{false link},mentions that are predicted as non-NIL and linked to the \ac{kr} while they were NIL referring to a new entity.
Errors from the NIL predictor introduce two critical problems in the pipeline. First, each false positive---or \emph{false NIL}, a non-NIL mentions predicted as NIL---produce a new entity. This adds redundant representations of the same entities to the \ac{newkr} and increases error propagation through batches, since all the mentions linked to these incrrect and redudant entities in the next time-steps are errors.
For this reason, to mitigate error propagation through time, it is preferable to achieve higher precision than higher recall, since \emph{false links} are errors but do not cause error propagation by populating the \ac{newkr} with redundant entities.

To visualize an overview of the process, \Cref{fig:clusters} shows that the number and size of clusters predicted by the pipeline is approximately as expected in the test set.
High precision and low recall translates in a lower number of clusters, for the most representing novel entities; while a high recall and low precition translates in a higher number of false representations of entities already included in the \ac{bgkr}.
...
Among the evaluated approaches the top-performing one is the 3-step; the reason could be that this method exploit a combination of lexical and semantic similarities between the mentions, while the other two ($\text{GNN}_\text{B}$ and $\text{GNN}_\text{F}$) only exploit semantic dense representations.
...
3-step & MUC F1       &  \textbf{96.84} &  \textbf{95.97} &  \textbf{95.89} &  \textbf{96.49} &  \textbf{96.96} &  \textbf{97.10} &  \textbf{96.94} &  \textbf{97.48} &  \textbf{97.26} &  \textbf{97.48} &  \textbf{96.84} & \textbf{97.38} \\
...
\hline
% & Greedy    &&&&&&&&&&&& \\
$\text{GNN}_\text{B}$ & MUC F1       &  86.48 &  84.22 &  84.55 &  87.65 &  89.23 &  89.60 &  89.30 &  91.07 &  90.88 &  91.05 &  88.40 & 91.56 \\
...
\hline
% & Feature    &&&&&&&&&&&& \\
$\text{GNN}_\text{F}$ & MUC F1       &  36.08 &  31.05 &  31.04 &  29.84 &  28.37 &  28.16 &  27.67 &  28.53 &  26.69 &  29.17 &  29.66 &  65.18 \\
...
3-step & CEAF F1      &  100.0 &  96.89 &  100.0 &  100.0 &  94.53 &  96.12 &  100.0 &  100.0 &  95.24 &  100.0 &  98.28 & \\
...
The results demonstrate that error propagation across batches is a key challenge in incremental entity extraction. A drop in performance can be especially observed on \ac{nel} results, as shown in \Cref{tab:superresults}: the results obtained in the first batch are comparable to the ones obtained in the single-pass ($\varnothing$) experiment and with the ``correct'' experiment, while they deteriorate in the following batches.
...
As previously explained, a major source of errors came from the false positives introduced by the management of NILs, suggesting that effective NIL prediction is an important challenge for the \ac{inel} task; in fact, low precision leads to the introduction of error into the pipeline and, therefore, to performance deterioration. % even if the classifier obtains relatively high precision levels (see \Cref{tab:superresults}) the error rate is high enough to degrade the performance in the subsequent batches.
...
Finally, in our experiments, we use a single vector for novel entity representation: the medoid obtained from the clusters of the NIL mentions. Evidence (see \Cref{tab:new_entities_recall}) show that this method is sub-optimal, despite being more efficient.
The problem of how to represent new entities, for instance using more than one vector per cluster or with different strategies (e.g. bounding-boxes), is another important challenge in the \ac{inel} task that is worth studying in order to find a better compromise between effectiveness and efficiency. We leave this exploration to future works.
% future work
