...
We evaluated the three baselines (that differ in the clustering approach) using these two experimental setup, although in \Cref{tab:superresults} we report only the NIL clustering performance for all the baselines, while the remaining metrics are obtained using the top-performing clustering approach ($3Steps$).
Finally, we run another experiment, called ``correct'', that corrects the output of previous components before proceeding through the batches and the pipeline, to better study the error-propagation. In \Cref{tab:superresults} we show the ``correct'' performance only of the top-performing pipeline ($3Steps$).
...
To visualize an overview of the process, \Cref{fig:clusters} shows that the number and size of clusters predicted by the pipeline is approximately as expected.
...
The results demonstrate that error propagation across batches is a key challenge in incremental entity extraction. A drop in performance can be especially observed on \ac{nel} results, as shown in \Cref{tab:superresults}: the results obtained in the first batch are comparable to the ones obtained in the one-pass ($\varnothing$) experiment and with the ``correct'' experiment, while they deteriorate in the following batches.
