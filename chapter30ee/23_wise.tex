...
\begin{table}[htbp]
  ...
  \begin{tabulary}{\textwidth}{LCCCCC}
    ...
    % \hline
    \textbf{2) Corruption} & 1442 & 1442  & 364690 & 63224 (51532-6273-4066) & 2351 \\
    \hline
  \end{tabulary}
...
During my \phd, I collaborated with prosecutors and judicial police officers from two public prosecutor's offices in the context of two separate investigations. While the work presented in this thesis refers exclusively to the second case---an investigation into suspected corruption---I report, in \Cref{tab:chatstats}, the statistics of the chat logs extracted and processed in both investigations, to better illustrate the volume of data that investigators must handle.
...
\subsection{Benchmark Annotation Process}
...
For \ac{nel}, we use the bi-encoder architecture of \blinkita considering the \ac{kr} otained from Italian Wikipedia---as in \Cref{sec:claw_consolidation}---additionally populated with the chat participants obtained with the metadata extraction from \ac{ima} data.\todo{introduce this before!}
...
and the knowledge graph is updated with the discovered entities---which correspond to the entity clusters---using the ``mentioned in'' relationship (see Figure~\ref{fig:model}).\todo{describe kg extraction and refer to next chapter}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l|cc|cc|c}
      ...
      & \textbf{Text}  & \textbf{Audio} & \textbf{to KB} & \textbf{NIL} & \\
      \midrule
      % \hline
