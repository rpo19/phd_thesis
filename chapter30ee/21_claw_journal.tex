...
Formally, they compare the observed agreement $P_o$ to the expected agreement $P_e$ under a chance model, and express reliability as
\begin{equation}
    (P_o - P_e)/(1 - P_e).
\end{equation}
...
\begin{table}[H]
    ...
    % \scalebox{.7}{
        ...
        \end{tabular}
        % }
    \caption{Types supported by the \ac{ner}, \ac{nel}, NIL prediction, and NIL Clustering algorithms. *Note that the Trie\ac{ner} relies on a gazetteer and it is potentially capable of extracting any entity regardless of its type as long as a pattern is provided. In our experiments, Trie\ac{ner} extracts entities of type \texttt{Person} and \texttt{Organization}, since the knowledge base (derived from documents metadata) of reference is limited to these two types.}
...
For NIL prediction, as in \textcite{ijckg2022} and \Cref{sec:ijckg:pipeline}, we use a \textit{logistic regression classifier} considering as input features:
\begin{enumerate}
...
The NIL clustering approach is the three-step clustering method described in \Cref{sec:ijckg:pipeline}, where thresholds are obtained with grid search using the same Wikipedia-based dataset used to train the \ac{nel} model.
