...
As a baseline, we consider the general-domain model \textit{ITA}---which is available pre-trained on huggingface\footnote{https://huggingface.co/dbmdz/bert-base-italian-xxl-cased}.\todo{maybe uniform the citation of hf models}
...
The comparison of backbone transformers for \ac{ner} considers five different random weight initializations for each backbone. We calculate the mean and the standard deviation of the micro \textit{precision}, \textit{recall}, and \textit{F$_1$-measure} of the five initializations for each transformer.
