...
In this chapter, we present Reliable Fact eXtractor (\refact), a constrained generation-based approach to \ac{qa} for addressing \ref{ob:refactx}, which require developing a scalable \ac{qa} system that can provide traceable and verifiable answers, grounded on external knowledge.
...
\textbf{Motivating Example.} The approach is illustrated in \Cref{fig:example}, by depicting an example with the question \textit{``When was the director of Slumdog Millionaire born?''}. The model is instructed to first reason on how to reach the correct answer, and then it starts to acquire facts.
% TODO fact command?
...
In this work we construct our \ac{kb} from Wikidata.
...
Intuitively, to constrain this process for generating only existing facts, we need to restrict $V$ to only allow tokens that can form a fact from the \ac{kb}. We define $V^A_{x_{<i}}$, which contains all the tokens that can lead to an existing fact if added to the sequence $x_{<i}$. Considering the example in \Cref{fig:constrained_generation}, at step $x_i$, when $x_{<i}=\text{``<Danny Boyle> <''}$, $V^A_{x_{<i}} = \{\text{``date''}, \text{``given''}\}$.
...
\begin{equation}
    ...
    V^A_{x_{<i}} = \mathrm{NT}_{\mathrm{kb}}(x_{<i}).
\end{equation}
...
While, the fact generated in normal mode is not correct, with constrained generation we are able to guide the \ac{llm} to the correct fact. The lower part of the figure details the mechanism of constrained generation. For each decoding step, allowed tokens are displayed in green boxes, whereas forbidden tokens are displayed in red boxes with dashed borders and a strike-through.
Tokens are arranged in ascending order according to $P_\theta(x_k = v)$, from bottom to top.

Starting from the sequence 
$x_{<i} = \text{``<Danny Boyle> <''}$, normal generation would generate $x_i = \text{argmax}_{v \in V}P_\theta(x_i = v \mid x_{<i}) = \text{``born''}$, leading to an incorrect fact, while with constrained generation $x_i = \text{argmax}_{v \in V}P^c_\theta(x_i = v \mid x_{<i}) = \text{``date''}$.
...
For obtaining the 800 million facts, we use the Wikidata truthy dump (Wikidata's highest-confidence statements, excluding qualifiers\footnote{\url{https://www.wikidata.org/wiki/Wikidata:Database_download/en\#RDF\_dumps}}) of the triples\footnote{The dump from 11 December 2024 was used.}.
...
Then, for each entity, we obtain a meaningful textual label (as Wikidata IDs lack meaning for \acp{llm}) corresponding to the Wikipedia title, if the entity is described in English Wikipedia\footnote{\url{https://en.wikipedia.org/}}. Otherwise, we use the template \verb|rdf-schema#label (schema.org/description)|, \eg ``Jane Hajduk (American actress)'', since the Wikidata label alone is not unique.
...
Additionally, while creating the tree, we calculate the number of reachable leaves from each node to avoid that \acp{llm} generate the same fact repeatedly, a behavior we witnessed during preliminary experiments.


\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{images/trie_example.drawio.pdf}
    \caption{Generating facts from the fact tree. Token ids are surrounded by rounded rectangles. The arrows $S \xrightarrow{n} v_k$ represent the selection of the next token $v_k$ from the current sequence $S=x_{<i}$ and $n$ is the number of leaves reachable from $S$.}
    % \vspace{2em}
...
In this way, during inference, subtrees that are manageable in size are directly loaded in memory (\eg with $L_c = 7$, $99\%$ of the subtrees use less than 116 \ac{kb}).
Secondly, we represent single-leaf sequences in a single row, as visible at the fourth row in \Cref{tab:postgres-table}, saving the rest of the sequence in \textit{Child.\#Lv.} (the token 29 comes after 694, and 662 is the last in the sequence).
%
...
Our prompt, shown in \Cref{fig:prompt}, instructs the \ac{llm} to, first, determine the reasoning path needed for answering, then to use the \emph{Fact} command for getting relevant facts from the \ac{kb}, and finally to answer based on the proofs acquired with the \emph{Fact} command. Additionally, we instruct the model to determine which kind of answer is required, to respond ``I don't know'' when no useful facts have been found, to understand when it is not able to find useful facts and stop, to strictly adhere to the prompt, and to be aware of the description predicate (often useful with the Wikidata \ac{kb}).
...
%
with the 800-million-fact tree derived from Wikidata indexed in PostgreSQL (see \Cref{sec:wikidata-repr}).
...
Precision and accuracy are calculated in two settings: (i) Exact Match: comparing the predicted answer with the ground truth with case-insensitive string equality; (ii) \ac{llm}-as-a-Judge~\cite{zheng2023llmjudge}: we ask Llama3.3-70B in 16-bit precision whether the predicted answer is correct and complete with respect to the ground truth.
...
We first evaluate the same \acp{llm} used with \refactx without constrained generation. In this setting, referred as \textit{\ac{llm}-only}, we use the same settings and prompts, so that models generates not-grounded facts based solely on their parametric-knowledge, giving us clues on how important is to access external knowledge and, furthermore, on how much each dataset can be answered using only the \ac{llm}'s internal knowledge.
%
...
\begin{figure}[h]
    ...
    \caption{Measured token generation time (in seconds). Results on 4,000 tokens, using Qwen2.5-3B with PostgreSQL running on the same machine, with key-value caching enabled, using one NVIDIA Tesla T4. We apply a moving average with a 10-tokens window to reduce noise.}
    % \vspace{2em}
