...
\begin{tabularx}{\textwidth}{p{1.1cm} X X X X}
\toprule
\textbf{Req.} & \textbf{Aspect} & \textbf{Ref. \cite{BELLANDI2024}} & \textbf{Ref. \cite{pozzi_wise_2025}} & \textbf{Ref. \cite{Agazzi2025DAVE}} \\
\midrule

Gener- & doc--ann--ent & \yes & \yes & \yes \\
aliza- & \ac{uri} & \psup not universal & \psup not universal & \psup not universal \\
bility & multi collect. & \almost not all \acp{ui} supp. & \no & \no \\
...
ability & visual. \ac{ui} & \almost no entity page & \yes & \yes \\

\midrule

Error & version/rollback & \no & \no & \no \\
Cor- & err corr. \ac{ui} & \psup no ent. no hist. & \psup no ent. no hist. & \psup no ent. no hist. \\
rection & ann. versioning & \psup multi doc vers. & \no & \no \\
\midrule

Scal- & net \ac{api} & \yes & \yes & \yes \\
ability & annset parallel. & \yes & \yes & \yes \\
...
In \textcite{BELLANDI2024} we built the first prototype that integrated storage, extraction, and \acl{ia} \acsp{ui} for civil judgments. The study explicitly considered architectural aspects, prototyped search and visualization over annotated judgments, adopted the document--annotation--entity representation, and showed generalizability and interoperability with ad-hoc services---\ie rule-based extractors (RAE)---for extracting law articles and postal addresses.

In \textcite{pozzi_wise_2025} we applied a similar design to investigative \ac{ima} data. The evaluation made clear that it was necessary to generalize the notion of document (chat vs.\ message) and that the existing implementation lacked support for multiple document collections---which would have been needed to represent both chats and messages within the same instance. In that prototype we instead used a \acl{kg} (implemented with neo4j~\cite{neo4j}, a graph database) to represent messages. The graph-based visualization provided in neo4j~\cite{neo4j} \ac{ui} received positive feedback from investigators.

Although, in that case, only a limited set of relations was considered, using a \acf{kg} enables storing entity facts as triples and increases the modeling capacity, as shown for instance by the \textit{participant-in} relation between \texttt{Person} and \texttt{Chat}. Such facts can also support more advanced \acl{ia} methods such as \refact, discussed in \Cref{chap:refactx}. Nevertheless, a \ac{kg} was not prescribed in the final architecture, since it is not strictly required under the minimum-necessity principle, and the \acf{kr} is a more general abstraction than a \ac{kg}.

Finally, \textcite{Agazzi2025DAVE} focused on improving \aclp{ui} for \acl{ia}, adding support for all retrieval modes. This work did not address other requirements such as multiple document collections, annotation versioning, or full error correction (\eg entity or document deletion was not available from the \acp{ui}).
