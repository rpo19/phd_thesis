...
With \textcite{pozzi_wise_2025}, the same design was stress-tested on investigative instant-messaging data, exposing the need for generalizing the notion of document granularity (chat vs.\ messages) and revealing the lack of support for multiple document collections in the existing implementation.\todo{speak of neo4j}
...
Finally, \textcite{Agazzi2025DAVE} explored a more \todo{evoluta} \ac{ia} interface and richer retrieval modalities, but still without addressing versioning, availability, peripheral services design principles, all explicitly mandated in the present architecture.\todo{improve}
...
& Visual. \ac{ui} & \almost no entity page & \almost no entity page & \almost no entity page \\

\midrule

ErrCorr. & version/rollback & \no & \no & \no \\
& ErrCorr. \ac{ui} & \almost, no ent. no hist. & \almost, no ent. no hist. & \almost, no ent. no hist. \\
\midrule
...
\multicolumn{2}{l}{Loose Coupling} & \yes & \no not addressed & \no not addressed \\
\midrule
...
\multicolumn{2}{l}{Multiple Collections} & \almost not all \acp{ui} supp. & \no & \no \\
\midrule

\multicolumn{2}{l}{Annotation Versioning} & \psup multi doc vers. & \no & \no \\
\midrule

Retrieval & retrieval modes & \psup sparse only & \psup sparse only & \yes \\
\midrule

\multicolumn{2}{l}{Query Language} & \yes SQL,MQL~\cite{mongodb-mql} & \yes MQL~\cite{mongodb-mql}, Cypher~\cite{neo4j-cypher} & \yes MQL~\cite{mongodb-mql} \\
\bottomrule
\end{tabularx}
\end{table}
