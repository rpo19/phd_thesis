\begin{table}[tb]
\centering
\caption[Comparison of the backbone transformers (one per row) for \ac{ner} on ICCJ146-EE test.]{Comparison of the backbone transformers (one per row) for \ac{ner} on ICCJ146-EE test. Using strong matching we calculate mean ($\pm$ std) on 5 random initializations.}
...
\toprule
            & \textbf{Precision}          & \textbf{Recall}             & \textbf{F$_1$}           \\ \midrule
