...
\todo{decoder only}
Also decoder-only models can be used used for language modeling\todo{ensure lm refers to clm}, as visible in \Cref{fig:transformer_all}(c), especially for tasks focusing a single language and vocabulary. Some examples are story telling 
However, modern decoder-only models are built with a vocabulary supporting dozens of languages~\cite{}, thus being suitable for translation tasks too.
