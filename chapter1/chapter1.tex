...
\subsubsection{Why useful in the era of LLMs}
...
Advances in extending the context window of \acp{llm}~\cite{}\todo{gemini 1m tokens. tesi di valentina} and on reducing the computational complexity reduced from quadratic to pseudo-linear time~\cite{}\todo{guarda il poster del 2 anno}.
\todo{maybe i should already introduce RAG?}
However, processing large volumes of text included in the context window of an \ac{llm} implies challenges such as the ``lost in the middle'' and the ``needle in a haystack'' problems~\cite{liu-etal-2024-lost,kuratov2024searchneedles11mhaystack}, where relevant information located far from the beginning or end of a long context may be overlooked by the model.
...
Furthermore, semantic search capabilities, such as faceted search, can be useful tools not just for humans but also for agentic \acp{llm}~\cite{}\todo{prendi da refactx qualcosa}, which can leverage them to access relevant information more effectively.
