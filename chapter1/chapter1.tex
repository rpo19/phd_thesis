...
\todo{maybe introduce a bit nlp wiht the objective and computational linguistic with the objective of capturing the meaning of text}
...
In the former case, annotating text with information from a \ac{kb} enriches the text with additional semantics, making it more machine-understandable, and enabling advanced information access functionalities, such as entity-based search and filtering via faceting~\cite{entityorientedsearch2018}.
In the latter case, \ac{ie} is used to populate or update a \ac{kb} with information extracted from unstructured sources, thereby enhancing the \ac{kb}'s coverage or keeping it up to date.

While \acsp{llm} possess high-level natural language understanding capabilities, \acl{sta} is still useful.\todo{find papers and motivate} 
...
\todo{also say that info extraction can add some kind of structure to unstructured web content}
\todo{for these reasons (verify makes sense) decades of research focused on info extraction (giving entity-based "structure"/order to unstructured content---using kbs to "structure"/make more machine understandable text) and knowledge base population (automatically populating/updating structured knowledge bases---using text for updating kbs)}
\todo{present info extraction also as an enrichment of text/unstructured data via semantic annotation}
