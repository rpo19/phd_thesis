...
\begin{definition}[Entity]
    An entity is a uniquely identifiable object, thing, or concept, characterized by its identifiers or names, type(s), attributes, and relationships to other entities or concepts.
\end{definition}
\todo{add entiy properties? or add some entity examples}

\begin{figure}[h]
\centering
% as of aug 25, 2025
\begin{lstlisting}[style=rdfstyle,caption={Excerpt from the DBpedia~\cite{dbpedia_auer_nucleus_2007} knowledge base entry of \textsc{Barack Obama}.},label={lis:obama}]
<dbr:Barack_Obama>
...
\end{lstlisting}
\end{figure}
\todo{something about triples and turtle? maybe later in the semantic web. make it turtle (missing ;?)}

The information about entities and their relationships can be stored and maintained in a \emph{\acl{kr}} or a \emph{\acl{kb}}~\cite{entityorientedsearch2018}.
%
\begin{definition}[\Acl{kr}]
    ``A \acf{kr} is a catalog of entities that contains entity type information, and (optionally) descriptions or properties
    of entities, in a semi-structured or structured format''~\cite{entityorientedsearch2018}.
\end{definition}
