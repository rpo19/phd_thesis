...
\begin{figure}[h!]
    ...
    \caption{Contextual embeddings of the word ``die'' calculated with BERT~\cite{devlin-etal-2019-bert}, projected in two dimensions. Figure from~\cite{cohen_2019_contextual_embeddings}.}
...
\begin{figure}[h!]
    ...
    \caption{The transformer encoder-decoder stack (applied to machine translation). Figure from~\cite{Alammar2018IllustratedTransformer}}
    \label{fig:transformer_encoder_decoder}
\end{figure}
%
Turning back to the transformer architecture, it is composed of a stack of encoders and a stack of decoders, as depicted in \cref{fig:transformer_encoder_decoder}. Each encoder and each decoder block receives a sequence of token embeddings as input and produces as output another sequence of embeddings.\todo{verify with jm3 and improve. add the picture from vaswani for describing the single block}
