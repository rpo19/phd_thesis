...
Besides the encoder-decoder architecture for machine translation, two relevant modifications are the \emph{encoder-only} and the \emph{decoder-only} transformers. The \emph{encoder-only} is especially used for embedding or sequence classification tasks, such as document retrieval with dense vector and named entity recognition~\cite{jm3,devlin-etal-2019-bert}, and it is trained with \ac{mlm} for learning to create useful token representations.
...
\textcite{devlin-etal-2019-bert} and \textcite{radford2018improving}, indeed, demonstrate that rich pre-training on generic language tasks, such as \acl{mlm} and \ac{clm}, is useful also for downstream tasks, e.g., \ac{ner} and \ac{qa}.
