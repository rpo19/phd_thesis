...
%
Given an input sentence of $N$ tokens, the sentence is divided in tokens, or \emph{tokenized}, then each token is assigned a static embedding from a matrix $E$ of shape $[|V|xd]$ where $V$ is the vocabulary of the model and $d$ the depth or number of dimensions of the embedding.
...
As visible in \cref{fig:transformer_architecture}, both the encoder (on the left side) and decoder modules (on the right) include attention layers.\todo{valuta se muovere dopo e aggiungi l'idea che i layer man mano arricchiscono le rappresentazioni con il contesto. parla di cross attention e magari cita bengio}

The transformer in figure targets machine translation. Suppose we are translating the French sentence ``Je suis étudiant'' to English, the expect output is ``I am a student''. The input tokens in French are fed to the network (bottom left) and the English output token ``I'' is produced (top right). Subsequently, the encoder (left module) receives the same starting tokens in French, while the decoder receives as input the already generated sequence of tokens in English, so far ``I'', resulting in the generation of ``am''. This process is continued iteratively until the model generates an end-of-sequence token or reaches the allowed limit of tokens to generate.\todo{valuta se cambiare immagine}



Both encoder and decoder block receive as input an embedding for each token and produce another embedding as output. Formally, let $X$ be the input matrix of shape $[Nxd]$. 
