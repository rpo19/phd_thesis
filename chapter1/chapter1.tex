...
While semi-structured repositories are well suited for human consumption, machines require more formalized representations. The \Acl{sw} community has been working to extend the Web into a machine-interpretable form~\cite{entityorientedsearch2018,decker_sw_rdf_xml_roles_2000}. This effort led to the development of the \acl{sw} stack, encompassing standards such as URIs, XML, \acs{rdf}, SPARQL, and ontology languages like RDFS and OWL~\cite{entityorientedsearch2018}.

The \acf{rdf} data model allows to represent an entity as a set of assertions or \emph{facts} about that entity. In this work, we will use \emph{fact} independenlty from the \ac{rdf} syntax, according to the following definition.
...
\end{definition}
Example of facts are ... (about Barack obama)
Instead in \ac{rdf} ...

\todo{definition of fact and examples}

knowledge in the form of triples---subject, predicate, object---where each element is typically identified by a URI. This simple but powerful structure allows entities to be uniquely referenced and linked across datasets, forming the foundation for interoperable, machine-readable knowledge on the Web~\cite{klyne2004rdf}.


\todo{while SW technologies can repr ents in structured form eg rdf, triples sub-pre-obj, kb kg, dbpedia, wikidata, freebase, yago (ontologies?)}


\todo{paraphrase and move KG after saying that they are equivalente but when viewed as a graph we use kg. also say why can be viewed as a graph. introduce also triples sub-pred-obj}
%
\todo{intrduce facts and triples and then move to the kb definition}
\begin{definition}[\Acl{kb}]
    ``A \acf{kb} is a structured knowledge repository that contains a set of facts (assertions) about entities''~\cite{entityorientedsearch2018}.
\end{definition}
%
\begin{figure}[h]
\centering
% as of aug 25, 2025
\begin{lstlisting}[style=rdfstyle,caption={Excerpt from the DBpedia~\cite{dbpedia,dbpedia_auer_nucleus_2007} knowledge base entry of \textsc{Barack Obama}.},label={lis:obama}]
<dbr:Barack_Obama>

    <dbo:birthName>    "Barack Hussein Obama II"
    <dbo:birthPlace>   <dbr:Honolulu>
    <dbo:birthDate>    "1961-08-04"
    <rdf:type>         <dbo:Person>
...
\end{figure}
%
Listing~\ref{lis:obama} shows some \emph{entity properties} for the DBpedia~\cite{dbpedia_auer_nucleus_2007} entity of \textsc{Barack Obama}, including types (``\textless dbo:Person\textgreater'', ``\textless dbo:Politician\textgreater''), attributes (``\textless dbo:birthName\textgreater'', ``\textless dbo:birthDate\textgreater'', ``\textless rdfs:comment\textgreater''), and relationships with other entities (``\textless dbo:birthPlace\textgreater'', ``\textless dbo:spouse\textgreater'').
...
% \begin{figure}[h]
% \centering
% \begin{lstlisting}[style=rdfstyle,caption={DBpedia entity: Rome},label={fig:rome}]
% <dbr:Rome>
%     <foaf:name>        "Rome"
%     <dbo:country>      <dbr:Italy>
%     <dbo:isPartOf>     <dbr:Lazio>
%     <dct:description>  "Capital city of Italy"
%     <dbo:populationTotal> "2860009"
%     <rdf:type>         <dbo:City>
%     <dct:subject>      <dbc:Capitals_in_Europe>
%     <dct:subject>      <dbc:World_Heritage_Sites_in_Italy>
%     <rdfs:comment>     "Rome is the capital and most populous city of Italy, famous
%                         for its history as the center of the Roman Empire, the Vatican,
%                         and its cultural and architectural landmarks."
% \end{lstlisting}
% \end{figure}
