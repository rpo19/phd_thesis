...
\begin{definition}[\Acl{kb}]
    A \acf{kb} is a structured form of \acl{kr} that organizes and stores facts about entities. 
    These facts are typically represented using data models such as \ac{rdf}~\cite{entityorientedsearch2018}.
\end{definition}

\todo{KG after saying that they are equivalente but when viewed as a graph we use kg. also say why can be viewed as a graph. introduce also triples sub-pred-obj}
