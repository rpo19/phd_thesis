...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{images/The_transformer_encoder_decoder_stack.png}
    \caption{The transformer encoder-decoder stack (applied to machine translation, French to English). Figure from~\textcite{Alammar2018IllustratedTransformer}}
    \label{fig:transformer_encoder_decoder}
\end{figure}
\todo{replace with mine original}
%
Turning back to the transformer architecture, it is composed of a stack of encoders and a stack of decoders, as depicted in \cref{fig:transformer_encoder_decoder}.
%
Given an input sentence of $N$ tokens, the sentence is divided in tokens, or \emph{tokenized}, then each token is assigned a static embedding from a matrix $E \in \mathbb{R}^{|V| \times d}$ where $V$ is the vocabulary of the model and $d$ the depth or number of dimensions of the embedding.
...
\begin{figure}[ht]
    \centering
    \includegraphics[height=10cm]{images/transformer_architecture.png}
    \caption{Overview of the transformer architecture. The encoder module is on the left, the decoder on the right. Figure from~\textcite{vaswani_transformers_2017}.}
    \label{fig:transformer_architecture}
\end{figure}
\todo{replace picture}


The transformer in \ref{} targets machine translation. Suppose we are translating the French sentence ``Je suis étudiant'' to English, the expect output is ``I am a student''. The input tokens in French are fed to the network (bottom left) and the English output token ``I'' is produced (top right). Subsequently, the encoder (left module) receives the same starting tokens in French, while the decoder receives as input the already generated sequence of tokens in English, so far ``I'', resulting in the generation of ``am''. This process is continued iteratively until the model generates an end-of-sequence token or reaches the allowed limit of tokens to generate.\todo{valuta se cambiare immagine o farne una e non citare jalammar. fai immagine (divisa in 3) encoder-decoder (machine translation), encoder-only (NER), decoder-only (text generation) with zoom on encoder and decoder block showing attention and ffn}

\todo{speak of encoder-only and decoder-only. who is better for what}

As visible in \cref{fig:transformer_architecture}, the encoder (on the left side) and decoder modules (on the right) include attention layers.\todo{valuta se muovere dopo e aggiungi l'idea che i layer man mano arricchiscono le rappresentazioni con il contesto. parla di cross attention e magari cita bengio}
...
\todo{continue with attention: self and cross}




Each encoder and each decoder block receives a sequence of token embeddings as input and produces as output another sequence of embeddings.\todo{verify with jm3 and improve. add the picture from vaswani for describing the single block}


and it is cabable of modeling sequences of tokens relying on the \emph{attention mechanism}, supported by positional encodings that provide information about the order of the tokens~\cite{vaswani_transformers_2017}.\todo{describe encoder decoder, attention and self attention}.
