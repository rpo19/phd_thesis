...
This section describes the first contribution of my Ph.D. work, published at the 11th International Joint Conference on Knowledge Graphs
(IJCKG 2022). In 2022, as described in \Cref{sec:incrementalel}, the advancements in the NIL prediction task were left behind the one in \ac{nel}. For instance, the BLINK approach~\cite{blink} was not yet adapted for NIL prediction.
\todo{expand motivation}
\todo{also add mammotab???}

Motivated by the context and by the relevance of incremental \ac{nel} in real-case scenario---where information is continuously updated---this work brought the following contributions to the research community: 
\begin{enumerate}
...
\subsection{A Baseline Pipeline for Incremental Entity Extraction}

% model overview
We propose a pipeline-based system composed by three modules: the NEL, the NIL prediction, and the NIL clustering modules.
A schema of the pipeline is available in figure \ref{fig:pipeline}: given a mention with its context, the Linking module retrieves the best candidate for linking the mention, then the NIL prediction module detects if the best candidate is correct, in which case the mention is linked to the BG-KB. Otherwise, the mention is NIL and is given, along with all the other NIL mentions found in the current batch, to the NIL clustering module that clusters together those mentions referring to the same new entity. At this point, each cluster corresponds to a new entity to add to the NEW-KB so that it can be retrieved when linking future mentions.
In this section, we further describe the pipeline components, and we finally discuss the representation of the novel entities. \newline
% Please notice that, in this asset, we strongly assume that the NEL module provides only rights couples entity-mention, if any: when the NIL predictor claims that the answer of the NEL is wrong, there is no check with other candidates following the first one.
% This assumption propagates the error of the NEL through the pipeline, although the HITL process may mitigate it.

\subsubsection{Linking}
%Given a mention with its context and a Knowledge Base, in our case the BG-KB and the NEW-KB combined, the linking module provides the $k$ best candidate entities for linking along with a confidence score for each entity.
% After call of April 05
The Linking module is based on the bi-encoder architecture \cite{Humeau2020Poly-encoders,blink,gillick-etal-2019-learning}, which is able to represent mentions (in the context) or entities (with a brief textual description) as dense vectors; at this point the linking score corresponds to the dot-product between mention and entity vectors.
%requires only a dense vector as a description.
% that encodes separately the entities or the mentions into vectors in the same space given, respectively, a title and a short textual description or the mentions in the context.
% At inference time the vector representation of all the entities in the KB can be pre-computed and cached \cite{blink}.
% The index is composed by a cached version of all the vectors, which can be queried in real time using the dot-product as a similarity score \cite{faiss}.

We use the bi-encoder for both candidate generation (through approximate nearest neighbor) and candidate ranking \cite{blink}.
%Linking with a bi-encoder essentially consists in calculating the vector representation of the mention and finding the nearest neighbors in the vector space that represent the KB entities.
%In fact, entities are embedded so that vector similarity is a good criteria to choose which entity to link to a mention.
%The use of a bi-encoder as candidate generator is well described by %\cite{blink}, which claims to obtain good performance in the retrieval step.
% dire come risolviamo il problema della descrizione con il bi encoder
%Taking advantage of the dense representation of the mentions, we are able to obtain a vector for a new entity even without a description.%, in order to obtain an estimation for the real value provided by the description.
%This process can be seen as the extrapolation of the meaning (of an entity) according to real-world examples.
In addition, the bi-encoder, behaving as a mapping function from the real-world examples to the vectors cached in the index, does not require any training step neither fine-tuning in case of KB updating (a drop in performance is expected): it is therefore well-suited for \textit{zero-shot NEL} since, as will be seen, from the dense representation it can be inferred the entity meaning in some ways analogue to the human understanding process.
%The use of a bi-encoder as a full NEL model, instead of a more complex and performing one, can be explained because it requires only a meaningful description of the entity, which can be extrapolated by the context, and does not require, by design, a fine-tuning in case of updating (it is well suited for \textit{zero-shot NEL}).
%Despite new entities are not well described, in fact, a vector can be extrapolated from examples of use (mentions) and then added to the index.
%In this way, the process can be compared to the extrapolation of a meaning (of an entity) according to real-world examples.

%\cite{blink} combined a bi-encoder with a cross-encoder \cite{humeau}, as a re-ranker, in their linking system since the latter is able to improve the linking performance.
%In our pipeline we omit the cross-encoder because it requires a textual description of the entity, which can not be obtained for new ones.
%A solution could be training a custom \textit{cross-encoder} that given two mentions detects whether they refer to the same entity of not. We leave this improvement to future works.
% without a description; however this aspect should be further investigated since it is not a trivial task, since there are a lot of unseen entities in the dataset which are included in the KB.
% @Riccardo anche se ieri ci siamo detti di metterla negli experiments espanderei la sezione approccio e la metterei a parte (prima o dopo dataset), mentre qui andrei solo con gli experiments. Va chiarito perchè non usiamo il cross-encoder
% @Fede: hai ragione, la finisco e la sposto prima del dataset

In the following experiments, we used the bi-encoder proposed as the candidate generator of BLINK \cite{blink}, available pre-trained on GitHub\footnote{\url{https://github.com/facebookresearch/BLINK}}.
For the Background KB we used the FAISS \cite{faiss} index based on the August 2019 Wikipedia dump and made available by \cite{blink}, making sure we filter out 
%after having removed
those entities we set as NIL for the experiments of section \ref{sec:experiments}.
Despite BLINK behave worse than RELIC \cite{RELIC, streaming_crossdoc_coreference} on all co-reference resolution tests, we opted for the first one as it is natively suited  for zero-shot NEL.\todo{add relic in el background}
%As index, we used the same FAISS \cite{faiss} index, previously modified in order to remove the entities which were set as NILs.

% (b) it is divided in two independent steps (candidate retrieval and re-sorting), easily permitting the inclusion of the other modules and (c) it offers a dense representation of the mention, which will be used for clustering.
% Since the candidate re-ranking step of BLINK (\textit{cross-encoder}) requires a meaningful description of the entity, We opted to remove this step, linking the mention with the most probable entity retrieved in the first step (\textit{bi-encoder}).
% Otherwise, the linking with new entities added in the previous steps would be impossible.
%The pipeline, at the end, is so composed:
%\begin{itemize}
%    \item bi-encoder (\textit{BLINK's pre-trained})
%    \item NIL predictor
%    \item NIL cluster
%\end{itemize}
...
%The NIL-prediction module predicts, from the candidate distribution, the correctness of the bi-encoder answer.
...
% Along with the linking score of the top-ranked candidate we consider additional features for the NIL prediction: the mean, median, and standard deviation of the scores of the top-k candidates, the Levenshtein and Jaccard similarity between the mention and the title of the top-ranked entity, and finally the NER types (PER, LOC, ORG, MISC) of the mention.
%from the candidate distribution, the correctness of the bi-encoder answer.
% To do so, a simple logistic regression on some features is applied: in this way, the answer has a score between 0 and 1 that can be used in a \textit{human in the loop} process.
% If the prediction is \verb|NIL|, the mention is then passed to the NIL clustering step in order to check if there are other mentions of the same entity.

Our model is based on a logistic regression whose input is composed by %extrapolated from the score distribution of the bi-encoder:
the linking score of the best candidate and the difference between the best and the second-best score. 
...
An additional study has been performed on the AIDA dataset \cite{aida} (in order to avoid overfitting and to obtain information about the entity type), also considering the types of both the mention and the top-ranked entity; this study highlighted that statistics are strongly dependent on the training KB thus not suited for I-NEL (that modifies the KB) and that types can give an important contribution to the NIL prediction, but unfortunately our dataset lacks typing information.
...
\subsubsection{NIL Clustering}
The NIL clustering module aims to group together the mentions referring to the same entity. %, providing, in addition, a dense representation.
The process is made by steps, considering both the \textit{surface form} and the embedding of the mentions.
...
% As result, for each cluster of mentions, the medoid vector is extrapolated, providing a description of the entity itself.
...
% \subsubsection{Feature-based / BLINK greedy nearest neighbour}
% The feature-based greedy nearest neighbour (GreedyF) and the BLINK greedy nearest neighbour (GreedyB) approaches were chosen based on promising result described in \cite{streaming_crossdoc_coreference}.
% Both these methodologies include a vectorizer: GreedyF uses one based on tf-idf, trained on the dev set of our dataset, while GreedyB uses the biencoder from BLINK. \todo{TF IDF, controllare}. The clustering thresholds have been calculated using binary search similarly to \cite{streaming_crossdoc_coreference}.
% %the preparatory tasks are two:
% % \begin{itemize}
% %     \item fitting a vectorizer using the dev set that will be used to calculate the scores later;
% %     \item finding a threshold using the binary search algorithm used by \cite{streaming_crossdoc_coreference}.% implementing also an early stopping technique that will be used by the clustering function;
% % \end{itemize}
% After the preliminary tasks there are three other steps: mention encoding, calculation of the similarity scores and clustering.
% In the feature-based approach the mentions are encoded in a way where there is the context composed by the join of the left and the right one and the mention itself. Mention similarity is a linear combination of mention text and contextual similarity as show in \cite{shrimpton-etal-2015-sampling}.
% In the BLINK approach, the mention encoding and the similarity score are standard as described in \cite{blink}.
% In both techniques the clusters are calculated starting from the scores of the previous step using a greedy nearest neighbour algorithm by finding its nearest neighbour in the sample, linking the two mentions if the similarity score is above the linking threshold calculated in the preparatory tasks as shown in \cite{shrimpton-etal-2015-sampling}.
% Lastly, the cluster found at the previous step are merged or split according to the distance between their medoid vectors.


%fine parte greedy

The third clustering method, $3Steps$, is a three-steps algorithm \cite{simone_24}: initially the mentions are clustered using their surface form (edit-distance $\leq 3$ for words longer than 3 characters, perfect match otherwise).
Then, inside each cluster we apply a hierarchical clustering algorithm, splitting to a predefined threshold, considering the mentions' dense vectors provided by the bi-encoder that correspond to a semantic representation of the mention.
...
\begin{figure}
    ...
    %Given a mention with its context, the Linking module provides the best candidates for linking, then the NIL prediction module checks whether these candidates are correct or if the mention is NIL (refers to an out-of-KB entity). In this latter case, the NIL Clustering module groups together the mentions referring to the same out-of-KB entity and finally this entity is added to the NEW-KB.
    }
    \label{fig:pipeline}
\end{figure}
% \todo{x @Riccardo}
% @Fede: l'immagine mi sembra poco chiara nella parte finale in cui le frecce escono dai riquadri LINK & NIL CLUSTER: così sembra che viene aggiornata anche la KG bergamasca (BG-KB), invece non c'è un vero e proprio flusso
% @ Riccardo la freccia di aggiornamento punta a NEW-KB


\paragraph{Novel Entities Representation}
%New entities, since are not included in the knowledge base, have no meaningful description.
...
%, in order to obtain an estimation for the real value provided by the description.
...
\begin{itemize}
    ...
    \item using the \textit{medoid} vector of the mentions of the cluster at the time in which it is stored into the KB;
    \item including in the KB \textit{all} the mentions vectors, as they were different entities.
\end{itemize}
...
\begin{table}
    ...
    %The dataset was changed in order to permit an easier management of the index, since AIDA has a lower number of observations.
    }
...
\end{table}
% Despite the latter strategy obtains the highest recall values, we decided to use the medoid vector to save computational time (it takes 5\% of the time) and disk space (it requires 22\% of the disk-space): this strategy indeed represents a good trade-off between efficiency %(near real-time use)
% and effectiveness.

Despite the latter strategy obtains the highest recall values, we decided to use the medoid vector to save resources. In fact it takes 5\% of the time and it requires 22\% of the disk space. This strategy indeed represents a good trade-off between efficiency %(near real-time use)
and effectiveness.

%\begin{table}
%    \centering
%    \caption{Time and disk required by the re-indexing of about 4000 new entities.}
%    \begin{tabular}{l|ccc}
%                        & vectors & time (seconds) & size (MB) \\
%     \hline
%         first / medoid &  4022 &  2.4 & 16 \\
%         all            &  18319 &  44.1 & 72
%    \end{tabular}
%    \label{tab:updating_cost}
%\end{table}

% Our preliminary results show that the inclusion of the new entity in the KB can happen in the first batch in which it appears; however other alternatives were not fully explored and should be investigated in future works.
%In the current asset, the novel entity is included into the KB the first time the NIL clustering algorithms finds it.
%Other alternatives, such as computing the confidence of the cluster, have not been explored and should be investigated in future works.
Despite the human intervention,
%(simulated by evaluating a new batch after having corrected the results of previous batches),
...
\subsection{Transforming a NEL Benchmark into an Incremental Entity Extraction Benchmark}

% comparazione dataset esistenti
% perché non usare un dataset per KG population standartd?
% perché a noi non interessano le relazioni tra le entità
%Since we are not interested in discovering relations between entities but only on identifying new ones and make them automatically linkable in future, we explored standard NEL datasets.
%However, they do not consider a temporal streaming of documents and most of them have limited focus on the identification of new entities. % \todo{verify or fix since AIDA may be seen as a couter-example: based on documents with NIL mentions (but does not add NIL mentions)} @Fede: this is (now) done, isn't it?
%Thus, they should be modified in order to include a significant number of new entities and split in batches to be adapted to the incremental extraction scenario. 
...
In order to test our pipeline in a realistic scenario, the chosen test dataset should be representative of characteristics that we expect to find in the real world. For example, the entity frequency distribution should have a long right-tail: there are a few popular (high-mentioned) entities, while most are not well-known; entities in train and test data should belong to the same domains (domain adaptation is an interesting problem, but is left out of this evaluation); the dataset should be big enough to easily train data-hungry models and to have a test set that can be split into several batches.
    %: few-shot and zero-shot learning should be investigated separately
...
In this paper we applied the following methodology to WNUM, because it is the only one freely available with the above mentioned features (e.g., AIDA is smaller than WNUM and has no ground truth on NILs) and uses links to Wikipedia entities, as well as most state-of-the-art NEL algorithms which use Wikipedia as reference KB \cite{blink,decao2021autoregressive,barba-etal-2022-extend}. Observe that even if Wikipedia may be not considered a proper KB, links between Wikipedia and Wikidata or DBpedia exists, in such a way that information about most of Wikipedia entities can be collected from proper KBs. 
%(avoiding the performance drop of the new domain, which can infer with model estimation).
In addition, it is designed so that each of the sets (train, dev, test) never contains a mention-entity pair that is present in another set, which is a highly appreciated property for our task.
...
\begin{figure}[htbp]
    ...
    \caption{Construction of the I-NEL dataset from a NEL dataset (a): first $p\%$ of mentions from the corpus are flagged as NILs and corresponding entities are removed from the KB (b); then observations are just \textit{transplanted} in order to obtain a well-represented distribution for the evaluation (c); finally the test set is split in batches (d).}
...
To evaluate the I-NEL we need to consider that, given a mention $m$ that refers to an entity $E$, the correct behavior may depend also on previous batches: in case $E$ is not present in the BG-KB at time $t_i$, $m$ should be classified as $NIL$; but, if one of the previous batches contained a mention of $E$, the system should have already added it to the NEW-KB and therefore $m$ should be linked to $E$.
Figure \ref{fig:evluation_outcome} summarizes how mentions should be processed according to the entity to which they refer.

The evaluation procedure for the I-NEL task should calculate the overall performance of a given system as well as 
...
\begin{itemize}
    \item NEL: we calculate $Recall@1$ for the candidate generation task; since the linking is made with the first candidate. Note that this metric coincides with accuracy.
...
In particular, remember that after clustering novel entities are added to the KB:
%a mention which should be linked to an entity added in the NEW-KB is considered correct in future steps if it is linked by the NEL module.
...
%In addition, the NIL prediction mitigates NEL errors, since, if the linking is wrong but the NIL predictor classify the result as a NIL entity, despite its presence in the BG-KB, the result of NIL predictor is considered as right.
%This because NIL entities are not included by definition in the BG-KB and therefore the NEL result is always wrong.
%Obviously, if the NIL predictor does not classify the mention of a novel entity as NIL, the linking is considered an error.

In addition, the NIL prediction mitigates NEL errors: if the linking is wrong, the NIL predictor would ideally classify the mention as NIL because the entity offered by the linker is not correct, despite the presence of the correct entity in the BG-KB.
...
\begin{figure}
    ...
    \caption{Schema of the expected outcome of the system, given a mention m referring to an entity E, when (a) E is known ``a priori'', (b) E has already been added while processing the previous batches, (c) E is not in the KB.}
    \label{fig:evluation_outcome}
\end{figure}
...
We can see that the performance of the NEL module is competitive compared to more complex models of state of the art: in particular, GENRE \cite{decao2021autoregressive} obtains a $R@1_{total} = 74.7$ and $R@1_{unseen} = 70.4$, which are quite similar to ours (excluding error propagation).
...
The drop in performance, as expected, is due to the error propagation in the incremental procedure and not to a poor NEL model. Indeed, in the ``correct'' experiment, there is no performance drop caused by the error propagation.
...
The NIL prediction component, despite the high precision, yields a high number of false negatives (entities that are predicted to be in the KB while they were new).
...
The results demonstrate that error propagation across batches is a key challenge in incremental entity extraction. A drop in performance can be especially observed on NEL results, as shown in table \ref{tab:superresults}: the results obtained in the first batch are comparable to the ones obtained in the one-pass experiment and with the ''correct`` experiment, while they deteriorate in the following batches.
...
As previously explained, a major source of errors came from the false positives introduced by the management of NILs, suggesting that effective NIL prediction is an important challenge for the I-NEL task; in fact, low precision leads to the introduction of error into the pipeline and, therefore, to performance deterioration. % even if the classifier obtains relatively high precision levels (see table \ref{tab:superresults}) the error rate is high enough to degrade the performance in the subsequent batches.
...
%&The purpose of the task is to reduce the human effort in updating existing KBs, automatically obtaining a representation of novel entities.
...
Our experiments show that a main challenge for I-NEL is to deal with the incremental propagation of error, which is due, especially, to the difficulty of linking entity mentions found in one batch to novel entities, i.e., entities not in the background KB and added to the NEW-KB in previous batches. 
...
% We presented our work as a strong baseline for the task of Incremental NEL (INEL), providing a modification of a ``static'' dataset \cite{Onoe_Durrett_2020} with a canonical order for data ingestion.
Indeed, the performance of the NEL component~\cite{blink} is reasonably good at the beginning but worsen over time. Another component that must be improved to deliver robust incremental NEL is NIL prediction.      
