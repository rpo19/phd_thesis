...
need to acquire and analyze evidence collected from a large number of heterogeneous sources, including structured, unstructured, and multimedia data~\cite{batini2021semantic}.

Considering the crucial role of entities in this domain, it is not surprising that semantic data integration and knowledge graphs (KGs)~\cite{knoblock2015exploiting} have been proposed as valuable assets to support investigative activities~\cite{szekely2015building,batini2021semantic}. Semantics is applied to interlink and organize data collected from heterogeneous sources, adopting an entity-centric data management approach.
...
Entities in the chat metadata (senders, participants) are parsed to construct the graph. For message content, we employ a Named Entity Recognition and Linking (NEEL) pipeline adapted from previous work~\cite{pozzi2023,pozzi23aixia}.
...
%Examples of effort-demanding activities are the analysis of semi-structured data such as mobile calls, bank transfers, credit card statements, and similar data. Based on the approach proposed in~\cite{batini2021semantic}, our co-authors, for example, applied wrapping mechanisms and knowledge graphs to support the analysis of credit card purchases for a number of investigated subjects that was not possible at a given cost. 
%
...
% \textbf{REMARKS}
% \begin{itemize}
%     \item SOTA from previous paper~\cite{batini2021semantic}: probably easier to modify this instead of writing this up from scratch?
...
%     \item Additional work on semantic / AI applied to criminal investigations (where we can reuse after summarizing what is written here); of course, cite~\cite{batini2021semantic}, but also ~\cite{kejriwal2017investigative,spyropoulos2023interoperability} ; probably also~\cite{alhajeri2023using}, \cite{bellomarini2020rule} (it uses a rule-based system)
%     \item For a discussion about the choice of the entity extraction algorithms (for the Italian language) used in our approach, we refer to papers where these algorithms have been discussed more in detail~\cite{pozzi2023,pozzi23aixia}. In these references we also discuss known limitations of these algorithms, identified especially in the accuracy of the NIL Prediction step. Entity clustering has been found as a less explored topic, which is currently under the attention of the NLP and semantic web communities~\cite{logan2021benchmarking,Heist2023nastylinker,kassner-etal-2022-edin}. In our work we have been inspired by~\cite{kassner-etal-2022-edin}, but we have been trying to improve it with additional features to compute mention-mention similarities and with community detection algorithms. A quantitative comparison between our clustering approach and previous work is out of the scope of this paper~\cite{}.      
% \end{itemize}
% per la scelta algoritmi ritorniamo agli altri paper
 


As discussed in Section~\ref{sec:introduction}, this work is an evolution of the approach described in~\cite{batini2021semantic} and is inspired by previous work to combat human trafficking based on semantic data integration~\cite{szekely2015building}. With respect to~\cite{batini2021semantic}, we specifically target IMA data, consider multimedia data enrichment, use a substantially improved NEEL pipeline, and include a semantic search interface. From~\cite{szekely2015building}, we borrow the overall entity-centric approach~\cite{kejriwal2018}, as well as the idea to improve search as a main objective~\cite{kejriwal2017investigative}. However, we work on IMA data with a native network structure rather than on data crawled from the web, and we apply a full-fledged NEEL pipeline to extract and interlink entities. Direct exploitation of graph queries and multimedia data enrichment are two additional novelties. % ontology
...
we refer to papers where these algorithms have been discussed more in detail~\cite{pozzi2023,pozzi23aixia}. In these references, we also provide a more precise comparison between our algorithms and other algorithms used in the legal domain, and we discuss some limitations of our pipeline. 
...
Most pipeline components for entity extraction directly derives from our previous work~\cite{pozzi23aixia},
...
in~\cite{pozzi23aixia}.


% \subsubsection{NEL}

For NEL, we use the bi-encoder architecture of BLINK~\cite{wu-etal-2020-scalable}, trained for the Italian language in previous work~\cite{pozzi23aixia}. The bi-encoder architecture encodes a mention and an entity separately into vectors and is optimized for maximizing the similarity between the vectors of correct mention-entity pairs.
...
In the NIL prediction component, we employ a \textit{logistic regression classifier} (the same as in~\cite{pozzi23aixia}).
It takes two inputs: 1) the NEL system's top-ranked entity score and 2) the score difference between the top-ranked entity and the second-best one. %~\cite{pozzi2023}. % TODO anon rimetti citazione
...
We annotated a gold standard for evaluating NER from six chats of which three are group chats, selected from the ones long enough and with audio transcripts. %We used the same methodology used in previous work~\cite{BELLANDI2024105904,pozzi23aixia}: % TODO anon riaggiungere citazione
...
% %In particular, 
% an ``approximate typed mention match" approach~\cite{pozzi23aixia}: an entity is ``recognized" if the annotation in the gold standard is included or includes our annotation and the predicted class is correct. Of the 285 entities present in the golden standard, 191 were correctly identified, leading to a score of 67\%. We focus on recall because of the search-driven exploitation of the annotations. 
% %With respect to precision, we found some mistakes on transcriptions related to I'm  

% Results shown in Table~\ref{tab:eval} are significantly worse than those obtained on other domain-specific data on which the pipeline we adapted was tested~\cite{pozzi2023,pozzi23aixia}. This confirms that NEEL on IMA data is challenging because of the specific data distributions and suggests that in-distribution fine-tuning is necessary. % to detect entities. % TODO anonymized. valutare se reinserire
...
Our current solution has several limitations. To overcome the weaknesses of pre-trained algorithms in the NEEL pipeline, we need to increase the size of the gold standard and fine-tune the algorithms on IMA data. %; based on our previous experience~\cite{pozzi2023}, we expect a significant improvement across all performance measures. % TODO anon. valutare se riaggiungere
%
