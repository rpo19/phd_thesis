...
Finally, the models, from each of the five configurations, have been fine-tuned for \ac{ner} using the SpaCy library with early stopping on the validation set and AdamW as the optimizer with the initial learning rate set to $5 \times 10^{-5}$.

\input{table/dataset_stats}

For the \ac{ner} fine-tuning data, we use the annotated corpus ICCJ146-EE, divided in three splits. The 30 documents also annotated for \ac{inel} represents the test set, while for the validation set we randomly select 12 documents from the remaining ones, ending up with 102 training documents.  
