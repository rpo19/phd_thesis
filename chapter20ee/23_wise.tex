...
Accordingly, this section focuses on the extraction and organization of entities from \ac{ima} data. Messages---including transcribed voice notes---were modeled within a knowledge graph designed to represent the network of contacts of the main suspect (\Cref{chap:architecture}). An improved version of the \acf{ee} pipeline introduced in \Cref{sec:eejudgements,sec:adaptation} was then applied and evaluated on a labeled corpus of chat logs. This corpus has been created through a procedure similar to the one employed for legal judgments in \Cref{sec:eejudgements}.
...
The work presented in this section is related to the \phd publication \textcite{pozzi_wise_2025}. Its main contributions, with respect to \acl{ee} applied to \acl{ima} data, can be summarized as follows:
\begin{itemize}
    \item Modeling of chat metadata in a knowledge graph to support graph-based investigative queries (\ref{uc:investigative-retrieval}).
    ...
    \item Assessment of \acf{ner} and incremental \acf{nel} on domain-specific data, identifying performance trends and key challenges.
\end{itemize}

The following sections describe the construction of the knowledge graph modeling chat metadata (\Cref{sec:wise:metdata_extraction}), the annotation process for obtaining labeled benchmark data (\Cref{sec:wise_dataset}), the methodology adopted for handling voice messages (\Cref{sec:wise_multimedia}), the entity extraction pipeline (\Cref{sec:wise_entity_extraction}), and the dataset, experiments, and evaluation procedures (\Cref{sec:wise_experiments}).
...
The entire schema of the \emph{chat \acl{kg}} is depicted in \Cref{fig:model}. It incorporates all the metadata present in chat dumps and it is predisposed to host the new entities identified from an \ac{inel} pipeline---namely \texttt{Location}, \texttt{Organization}, \texttt{Date}, \texttt{Money}, and \texttt{Miscellaneous}, all of which are subclasses of \texttt{Entity}, together with \texttt{Person}. Finally, the \emph{mentioned in} relation allows persisting the association between entities and the messages and chats in which they are mentioned. Consequently, investigators can trace entities to the source messages and verify the correctness of potential deduction reading the original messages (\ref{ch:traceability}). 
...
\begin{itemize}
  ...
  \item and optionally the attachments it contains.
\end{itemize}
...
\begin{table}[htbp]
  ...
  \begin{tabulary}{\textwidth}{LCCCCC}
    ...
    \hline
  \end{tabulary}
  \caption[Statistics about two investigations.]{Statistics about two investigations in which I have been consultant. The work described in this section refers only to the second investigation about suspected corruption.}
  \label{tab:chatstats}
\end{table}

\Cref{tab:chatstats} shows the resulting statistics of the metadata extraction in both the investigations I was involved. The remainder of this section refers solely to the second investigation about suspected corruption.
...
while NIL clustering has been improved for this study. 
...
% for the Italian legal domain and finally for \ac{ner}
% in~\cite{aixia_2023}. 
...
For \ac{nel}, we use the bi-encoder architecture of \blinkita considering the \ac{kr} obtained from Italian Wikipedia, as in \Cref{sec:claw_consolidation}, extended with the chat participants from the chat \ac{kg}.
...
%are the main targets for linking in the investigative context, as the references to known organizations and locations can help disambiguate references and link messages across different chats.

% \subsubsection{NIL Prediction}
In the NIL prediction component, we employ the logistic regression classifier described in \Cref{sec:claw_consolidation}.
It takes the \ac{nel} top-ranked score and ``secondiff'' (the difference between the top-ranked score and the second-best) and produces a probability $p \in [0,1]$ as output, where 1 denotes the top-ranked link is correct and 0 assigns NIL.

        % \subsubsection{Entity Clustering and Consolidation}


The entity clustering approach, which has been improved with respect to previous work in \Cref{sec:eejudgements}, uses a supervised XGBoost classifier~\cite{chen2016xgboost}, trained with the mentions from the labeled \ac{ima} dataset, to calculate a similarity score between pairs of mentions. The input features include lexical similarity measures, namely Jaro-Winkler~\cite{winkler1990string} and Jaccard~\cite{jaccard1901etude}, as well as semantic similarity computed as the cosine similarity of the mention vectors obtained from the \blinkita bi-encoder. The classifier outputs a synthetic similarity score.  
...
\begin{enumerate}
  \item Demonstrating that the combination of graph-based modeling, multimedia enrichment, and NLP-based entity extraction enhances \ac{ima} data analysis in criminal investigations through graph-based querying and semantic search.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l|cc|cc|c}
      ...
      \bottomrule
    \end{tabular}
    \caption[Statistics of the \ac{inel} extraction from chats.]{Statistics of the \ac{inel} extraction from chats. *Links for Person refer to the chat knowledge graph, not to Wikipedia.}
    \label{tab:stats}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
      ...
      \bottomrule
    \end{tabular}
    \caption[\ac{ner} evaluation on the chat dataset.]{\ac{ner} evaluation on the chat dataset. \textit{Strong} counts perfect matches of span and type. \textit{Partial} counts as correct when there is an overlap between predictions and ground truth.}
    \label{tab:eval}
\end{table}
...
%At the time of writing this paper, we are still working on finalizing the interconnection between the \ac{inel} pipeline and the knowledge graph. For this reason, we can't yet provide any additional statistics on the number of different ``entity" nodes.
...
% We annotated a labeled dataset for evaluating \ac{ner} from six chats of which three are group chats, selected from the ones long enough and with audio transcripts. %We used the same methodology used in previous work~\cite{BELLANDI2024105904,aixia_2023}:
...
% Results shown in Table~\ref{tab:eval} are significantly worse than those obtained on other domain-specific data on which the pipeline we adapted was tested~\cite{ijckg2022,aixia_2023}. This confirms that \ac{inel} on \ac{ima} data is challenging because of the specific data distributions and suggests that in-distribution fine-tuning is necessary. % to detect entities.
...
On the other hand, metadata are precisely identified enabling accurate filtering with faceted search.
...
%For this reason, we can't yet provide any additional statistics on the number of different ``entity" nodes.
