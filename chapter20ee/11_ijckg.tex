...
and $N_{\text{BGKR}}$ is the total number of mentions whose gold target is in the \ac{bgkr}.
...
and $N_{\text{NIL}}$ is the number of mentions whose gold label is NIL.
...
and $N_{\text{NEWKR}}$ is the corresponding gold count.
...
For NIL prediction, modeled as a binary classification, we compute precision, recall and $F_1$ on the NIL class:
\begin{equation}
...
% \end{equation}
\paragraph{NIL Clustering}
For NIL clustering we follow the standard co-reference evaluation formulation
...
that participate in this optimal mapping:
\begin{equation}
...
\medskip
Finally, the $F_1$ scores are calculated with the harmonic mean, as for NIL prediction, and we calculate the macro average of the $F_1s$:
\begin{equation}
F_{1,\text{avg}} =
\frac{F_{1,\text{MUC}} + F_{1,\text{B}^3} + F_{1,\text{CEAF}_e}}{3}.
\end{equation}
...
With respect to NIL clustering, \Cref{fig:clusters} shows the number of predicted clusters as a function of their size, together with the expected distribution. The general agreement between the two indicates that the pipeline does not overproduce clusters, and that the high precision yields cluster sizes broadly consistent with the expected values. However, the observed MUC score of 100.0 may be explained by the large proportion of singleton clusters~\cite{recasens_hovy_2011}.
