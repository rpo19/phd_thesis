...
\section[Adapting an Entity Linking Benchmark to Incremental Entity Linking][Adapting an \acs{nel} Benchmark to \acs{inel}]{Adapting an Entity Linking Benchmark to Incremental Entity Linking}
\label{sec:ijckg}

% \acl{inel}. In this setting, given $N$ batches of textual documents $b_0, \ldots, b_N$ corresponding to $N$ time steps, and an initial \ac{kr} $\mathrm{KR}_0$, each batch $b_k$ is processed to identify NIL entities, which are then added to the \ac{kr} to obtain $\mathrm{KR}_{k+1}$. The updated \ac{kr} $\mathrm{KR}_{k+1}$ is subsequently used as the reference knowledge resource for processing the next batch $b_{k+1}$.



% The work presented in this section was published in 2022 at the 11th International Joint Conference on Knowledge Graphs (IJCKG 2022)~\cite{ijckg2022}.
%
% The proposed approach addresses the \nilchallenge by extending \ac{nel} with a background \ac{kbr} with NIL prediction, novel entity clustering, and the incremental population of the \ac{kr} with novel entities. We apply this methodology to an existing benchmark, propose a baseline pipeline, and demonstrate the increased difficulty of \acl{ee} in incremental settings.
% It focuses on batch-based \acl{inel}, proposing a procedure to adapt existing \ac{nel} datasets to this setting. We applied this method to the WNUM \ac{nel} dataset~\cite{Onoe_Durrett_2020} obtaining its incremental version, \emph{Incremental WNUM}, available from GitHub~\footnote{https://github.com/rpo19/Incremental-Entity-Extraction}.
%
% When the work has been published, in 2022, the advancements in the NIL prediction task were left behind the one in \ac{nel}, as described in \Cref{sec:incrementalel}. For instance, the BLINK approach~\cite{blink} was not yet adapted for NIL prediction.
% For this reason we addressed the \nilchallenge, working towards producing a dataset and an evaluation procedure for aiding the research in this field.

% Following this line, during my \phd I co-authored \emph{\mammotab}~\cite{mammotab}, a large-scale dataset of 1 million annotated Wikipedia tables extracted from 20 million pages and aligned with Wikidata. \mammotab is not part of this thesis but still addresses a related topic, \acf{sti}---the field that studies interpretation tasks for tabular data, that include linking cells containing entity mentions to the correct entry in a \ac{kr} (similarly to \ac{nel} for text), identifying the types of table columns, and the relations between different columns of the table~\cite{cremaschi2024surveysemanticinterpretationtabular}. Among the challenges addressed, \mammotab was also constructed for the \nilchallenge, including NIL-labeled instances. This work has been published in the Proceedings of the Fourth edition of the Semantic Web Challenge on Tabular Data to Knowledge Graph Matching (SemTab 2022)~\cite{mammotab}.



In this section, we address \ref{ob:extraction_assessment}, especially focusing on the \nilchallenge, by evaluating and adapting existing \acl{nel} techniques to \acf{inel}---which extends \ac{nel} techniques to an incremental scenario where NIL mentions are identified, clustered, and finally added to the reference \acf{kr} (see \Cref{sec:incrementalel} for more).

Indeed, entities in domains such as law or healthcare are often unknown in public \acp{kr}, such as Wikipedia. Examples are the individuals mentions in court judgments, the suspects of an investigation, or patients in healthcare.

The contributions in this section relate to the \phd publication \textcite{ijckg2022} and can be summarized as follows.
\begin{enumerate}
...
The following sections describe, in order, the baseline pipeline with the motivations behind the choice of each component (\Cref{sec:ijckg:pipeline}), the procedure to adapt an \ac{nel} dataset to the incremental setting and its application to WNUM~\cite{Onoe_Durrett_2020} (\Cref{sec:ijckg:dataset}), followed by the definition of an incremental evaluation procedure (\Cref{sec:ijckg:evaluation_procedure}) an experimental evaluation of the baseline pipeline on the incremental-WNUM (\Cref{sec:ijckg:experiments,sec:ijckg:results}), and a discussion of the results (\Cref{sec:ijckg:discussion}).

\subsection{A Baseline Pipeline for Incremental Entity Linking}
\label{sec:ijckg:pipeline}

The proposed baseline pipeline consists of three main modules: \acl{nel}, NIL prediction, and NIL clustering. As illustrated in \Cref{fig:pipeline}, the process operates as follows: given a mention and its context, the linking module retrieves the most suitable candidate from a \emph{\acf{bgkr}}. The NIL prediction module then verifies whether this candidate is correct. If so, the mention is linked to the \ac{bgkr}; otherwise, it is marked as NIL, assuming that the correct candidate is not present in the \ac{kr}. All NIL mentions in the current batch are subsequently passed to the NIL clustering module, which groups together those referring to the same unseen entity. Each resulting cluster corresponds to a new entity, which is added to the \emph{\acf{newkr}} to support future linking tasks. Although the distinction between \ac{bgkr} and \ac{newkr} is not strictly necessary---as \ac{inel} could also be achieved by storing all entities in a single reference \acl{kr}---we maintain this conceptual separation for greater clarity.
...
\begin{figure}[htbp]
    ...
    {images/ijckg/pipeline_simple.drawio.pdf}
    \caption[Schema of the \acs{inel} pipeline.]{Schema of the \acl{inel} pipeline.
    }
    \label{fig:pipeline}
\end{figure}

\paragraph{Entity Linking} The \ac{nel} module relies on a bi-encoder architecture~\cite{Humeau2020Poly-encoders,blink,gillick-etal-2019-learning}, which retrieves candidate entities through \acl{ann} search and scores mention--entity pairs using similarity measures such as the dot product.

In our experiments, we used the pretrained BLINK~\cite{blink} bi-encoder, publicly available on GitHub\footnote{\url{https://github.com/facebookresearch/BLINK}}. For the \ac{bgkr}, we adopted the FAISS~\cite{faiss} index built from the August 2019 Wikipedia dump released by \textcite{blink}, ensuring that entities designated as NIL for the experiments were excluded.
...
\bottomrule
\end{tabular}
    \caption[NIL prediction feature ablation study.]{NIL prediction feature ablation study on the WNUM~\cite{Onoe_Durrett_2020} dataset, using the training set for model fitting and the development set for evaluating. \textit{max} is the score of the best candidate, \textit{levenshtein} and \textit{jaccard} are textual similarities between the mention and the entity title, \textit{secondiff} is the difference between the best and the second-best score.}
...
The second method, \textit{$\text{GNN}_\text{F}$}, also relies on GNN but uses a feature-based vectorizer following \textcite{shrimpton-etal-2015-sampling}. Mentions are encoded as character skip-bigram indicator vectors for surface text and TF-IDF~\cite{jm3} vectors for context.

The third method, referred to as \emph{three-step clustering (3-step)}, follows \textcite{simone_24}. First, mentions are grouped by surface similarity: mentions with an edit distance $\le 3$ (using the Damerau-Levenshtein edit distance~\cite{damerau1964}) are clustered together, except for short mentions of less than 3 characters that are clustered only if identical. This step groups similar mentions but may combine semantically different entities (\eg \texttt{Lisbon (1956 film)} and \texttt{Lisbon, Capital of Portugal}). Next, we apply single-linkage agglomerative hierarchical clustering~\cite{Rokach2005} within each group based on the semantic similarity calculated with the embeddings from the bi-encoder, to separate polysemous mentions. Finally, each sub-cluster is represented by its medoid vector, and semantically close sub-clusters are merged based on their medoid similarities, with another single-linkage agglomerative hierarchical clustering pass.
...
\begin{itemize}
    ...
    \item or including \textit{all} the mention vectors in the \ac{kr}, treating them as separate entries. In this case, multiple entries may correspond to the same entity.
\end{itemize}

In this experiment, all the mentions from the AIDA~\cite{aida} training set have been added to the \ac{kr} according to the three strategies.
\Cref{tab:new_entities_recall} reports the recall values obtained by the bi-encoder linking the mentions from AIDA test$_a$, for each strategy, together with the corresponding time and storage requirements, as measured on the AIDA dataset~\cite{aida}.
Although including all mention vectors yields the highest recall, we adopt the \textit{medoid} strategy as a compromise between efficiency and effectiveness. It achieves comparable performance while requiring only 5\% of the time and 22\% of the storage compared to the \textit{all} strategy.
\begin{table}[!htbp]
\centering
\begin{threeparttable}
    \caption[Recall@k \acs{nel} results with different representations for NIL entities.]{Recall@k \ac{nel} results between different representation strategies for NIL entities, with retrieval time and the disk usage. The \acl{kr} has been built using the mentions in the AIDA~\cite{aida} training set, while the results refer to linking the mentions from AIDA test$_a$. The indexes were created using FAISS~\cite{faiss} IndexFlatIP class\tnote{1}.}
...
% \subsection{Transforming an \ac{nel} Benchmark into an Incremental Entity Extraction Benchmark}
...
In this work, we set $p = 0.1$ to obtain 10\% of NIL entities, following~\cite{agarwal_2021_arborescence_arxiv} and apply a Bernoulli trial with probability $p_{NIL}(e)$ to decide, for each $e$, whether to flag it as NIL. Entities marked as NIL are removed from the \ac{bgkr} and treated as unknown. To increase the number of new entities in the development and test sets, some mentions of NIL entities are randomly transplanted from the training set. Both development and test sets are assigned 500 new mentions each to ensure robust evaluation (see \Cref{tab:dataset_nils}).
%
Finally, the test set is split into 10 batches using stratified sampling based on entity frequencies and NIL status. \Cref{fig:incel_dataset_construction} displays the entire dataset adaptation process, while detailed per-batch statistics are available in \Cref{tab:dataset_stats_extended}.

\begin{table}[htbp]
    \centering
    \caption{Statistics about the \acs{inel} dataset before and after the \textit{transplant}.}
...
\begin{table}[htbp]
    \centering
    \caption[Per-batch statistics about the \acs{inel} test set.]{Per-batch statistics about the \acs{inel} test set: number of mentions, entities, NIL mentions, new entities, and \emph{Prev~Entities}: new entities already found in a previous batch (that should be linked to an entity previously added to the \ac{newkr})}
...
\begin{figure}[htbp]
    ...
    \includegraphics[
        width=\textwidth,
        keepaspectratio]{images/ijckg/incel_construction.drawio.pdf}
    \caption[Construction of the \ac{inel} dataset.]{Construction of the \ac{inel} dataset from an \ac{nel} dataset (a): first $p\%$ of mentions from the corpus are flagged as NILs and corresponding entities are removed from the \ac{kr} (b); then observations are just \textit{transplanted} in order to obtain a well-represented distribution for the evaluation (c); finally the test set is split in batches (d).}
    \label{fig:incel_dataset_construction}
\end{figure}
...
To evaluate \ac{inel}, we must consider that the correct handling of a mention $m$ referring to an entity $e$ may depend on previous batches. If $e$ is not present in the \ac{bgkr} at time $t_i$, $m$ should be classified as NIL. However, if a previous batch already contained a mention of $e$, the system should have added it to the \ac{newkr}, and $m$ should be linked to $e$. 

The evaluation procedure for the \ac{inel} task should measure both the overall system performance and the performance on subsets of mentions that, in each batch, need to be:
\begin{enumerate}[label=(\alph*)]
  \item linked to the \ac{bgkr};
  \item classified as NIL;
  \item linked to the \ac{newkr}.
\end{enumerate}
\begin{figure}
    \centering
    \includegraphics[
    width=.5\textwidth,
    keepaspectratio]
    {images/ijckg/experiments_expected.drawio.pdf}
    \caption[Schema of the expected outcome of an \acs{inel} system.]{Schema of the expected outcome of the \acs{inel} system, given a mention $m$ referring to an entity $e$, when (a) $e$ is known in the \ac{bgkr}, (b) $e$ has already been added to the \ac{newkr} while processing the previous batches, (c) $e$ is not in \ac{bgkr}, nor in \ac{newkr}.}
...
\Cref{fig:evluation_outcome} summarizes how mentions should be processed depending on the entity they refer to.




\subsubsection{Evaluation Measures} \label{sec:inel_metrics}

Evaluating an incremental pipeline is more complex than evaluating a single model, 
since errors propagate both through the pipeline and across time (i.e., batches).
First, we define measures for the overall \ac{inel} pipeline:

\begin{enumerate}[leftmargin=*, label=(\alph*), ref=(\alph*)]

\item \textbf{Link to \ac{bgkr}:} accuracy for mentions that should be linked to the \acl{bgkr}:
\begin{equation}
\text{Acc}_{\text{BGKR}} =
\frac{TP_{\text{BGKR}}}{N_{\text{BGKR}}}
\end{equation}
where $TP_{\text{BGKR}}$ is the number of mentions correctly linked to the \ac{bgkr}, 
and $N_{\text{BGKR}}$ is the total number of mentions referring to entities in the \ac{bgkr}, according to the ground truth.

\item \textbf{NIL:} accuracy for mentions that should be classified as NIL:
\begin{equation}
\text{Acc}_{\text{NIL}} =
\frac{TP_{\text{NIL}}}{N_{\text{NIL}}}
\end{equation}
where $TP_{\text{NIL}}$ is the number of mentions correctly predicted as NIL, 
and $N_{\text{NIL}}$ is the number of mentions whose ground truth label is NIL.

\item \textbf{Link to \ac{newkr}:} accuracy for mentions linked to the \ac{newkr}:
\begin{equation}
\text{Acc}_{\text{NEWKR}} =
\frac{TP_{\text{NEWKR}}}{N_{\text{NEWKR}}}
\end{equation}
where $TP_{\text{NEWKR}}$ is the number of mentions correctly linked to entities previously added to the \ac{newkr},
and $N_{\text{NEWKR}}$ is the total number of mentions that, according to the ground truth, refer to entities previously added to the \ac{newkr}, and thus should be linked to it.


\item \textbf{Overall Accuracy:} accuracy across all mentions:
\begin{equation}
\text{Acc}_{\text{overall}} =
\frac{TP_{\text{BGKR}} + TP_{\text{NIL}} + TP_{\text{NEWKR}}}
     {N_{\text{BGKR}} + N_{\text{NIL}} + N_{\text{NEWKR}}}.
\end{equation}

\end{enumerate}

\medskip\noindent
Secondly, to understand how each component of the pipeline performs on its specific task, 
we also evaluate the modules separately.

\paragraph{Entity Linking (NEL)}
For the \ac{nel} module we report Recall@1, as in \textcite{blink}, which 
counts a prediction as correct when the top-ranked entity matches the reference one. Formally:
\begin{equation}
\text{Recall@1} = 
\frac{\#\{\text{mentions with correct top prediction}\}}
     {\#\{\text{all linked mentions}\}}.
\end{equation}

\paragraph{NIL Prediction}
For NIL prediction, which is modeled as a binary classification, we compute precision, recall and $F_1$ on the NIL class:
\begin{equation}
\text{Precision}_{\text{NIL}} = 
\frac{TP_{\text{NIL}}}{TP_{\text{NIL}} + FP_{\text{NIL}}}, \qquad
\text{Recall}_{\text{NIL}} =
\frac{TP_{\text{NIL}}}{TP_{\text{NIL}} + FN_{\text{NIL}}}.
\end{equation}
\begin{equation}
F_{1,\text{NIL}} =
\frac{2 \cdot \text{Precision}_{\text{NIL}} \cdot \text{Recall}_{\text{NIL}}}
{\text{Precision}_{\text{NIL}} + \text{Recall}_{\text{NIL}}}.
\end{equation}

% \paragraph{NIL Clustering}
% For NIL clustering we follow the standard coreference resolution evaluation methodology~\cite{streaming_crossdoc_coreference} and compute 
% MUC, B$^3$, and CEAF$_e$ precision, recall and $F_1$, as well as the macro-averaged of the $F_1$ scores. These metrics capture complementary 
% aspects of cluster quality: (i) MUC measures how many reference links between mentions are recovered; 
% (ii) B$^3$ evaluates correctness at mention level via cluster overlap; 
% (iii) CEAF$_e$ finds the best one-to-one alignment between predicted clusters and reference clusters 
% and scores their overlap.

% We finally report the average $F_1$ across the three metrics:
% \begin{equation}
% F_{1,\text{avg}} =
% \frac{F_{1,\text{MUC}} + F_{1,\text{B}^3} + F_{1,\text{CEAF}_e}}{3}.
% \end{equation}
\paragraph{NIL Clustering}
For NIL clustering we follow the standard coreference evaluation metrics
of MUC~\cite{vilain1995}, B$^3$~\cite{bagga-baldwin-1998}, 
and CEAF$_e$~\cite{luo2005coreference}, as in \textcite{streaming_crossdoc_coreference}.  
Let $K=\{k_i\}$ be the set of annotated clusters and $R=\{r_j\}$ the set of predicted clusters. 

\medskip
\noindent\textbf{MUC}~\cite{vilain1995} measures link recovery:
\begin{equation}
\text{Recall}_{\text{MUC}} =
\frac{\sum_{k_i \in K} \left( |k_i| - |p(k_i)| \right)}
     {\sum_{k_i \in K} \left( |k_i| - 1 \right)}, \qquad
\text{Precision}_{\text{MUC}} =
\frac{\sum_{r_j \in R} \left( |r_j| - |p(r_j)| \right)}
     {\sum_{r_j \in R} \left( |r_j| - 1 \right)} ,
\end{equation}
where $p(k_i)$ is the set of partitions obtained by intersecting $k_i$ with $R$ (and symmetrically for $p(r_j)$).

\medskip
\noindent\textbf{B$^3$}~\cite{bagga-baldwin-1998} evaluates mention-wise overlap:
\begin{equation}
\text{Recall}_{\text{B}^3} =
\frac{
\sum_{k_i \in K} \sum_{r_j \in R} \frac{|k_i \cap r_j|^2}{|k_i|}
}{
\sum_{k_i \in K} |k_i|
}, \qquad
\text{Precision}_{\text{B}^3} =
\frac{
\sum_{k_i \in K} \sum_{r_j \in R} \frac{|k_i \cap r_j|^2}{|r_j|}
}{
\sum_{r_j \in R} |r_j|
}.
\end{equation}

\noindent\textbf{CEAF$_e$}~\cite{luo2005coreference} aligns clusters one-to-one by maximizing a similarity $\phi$.  
We use the entity-based similarity $\phi_e(k_i,r_j)= \frac{2\,|k_i\cap r_j|}{|k_i|+|r_j|}$. Let $g^*$ be the optimal
one-to-one mapping of clusters (maximizing $\sum \phi$), and let $K^*$ be the subset of annotated clusters
that participate in this optimal mapping. Then, we define:
\begin{equation}
\Phi = \sum_{k_i \in K^*} \phi_e(k_i, g^*(k_i)).
\end{equation}
Precision and recall are computed as:
\begin{equation}
\text{Recall}_{\text{CEAF}_e} =
\frac{\Phi}{\sum_{k_i \in K} \phi_e(k_i,k_i)}, \qquad
\text{Precision}_{\text{CEAF}_e} =
\frac{\Phi}{\sum_{r_j \in R} \phi_e(r_j,r_j)}.
\end{equation}
...
Note that the handling of a mention $m$ referring to an entity $e$ previously added to the \ac{newkr} is considered correct only if it consists of linking $m$ to $e$, while labeling $m$ as NIL is considered an error. This ensures that the system does not achieve artificially high scores by treating every mention as a new entity.

NIL prediction outcomes can also mitigate \ac{nel} errors. If the linking is incorrect, the NIL predictor should ideally classify the mention as NIL because the suggested entity is wrong---even if the correct entity exists in the \ac{bgkr}. Conversely, if the NIL predictor fails to classify a new entity as NIL, it is considered an error.
...
To enable comparison with other models, we tested our baselines using two experiments. The first experiment evaluates the models on the entire unbatched test set in a \emph{single pass} ($\varnothing$), performing only one step of clustering. 
...
With respect to NIL clustering, \Cref{fig:clusters} shows the number of predicted entity clusters as a function of their size, together with the expected distribution. The general agreement between the two indicates that the pipeline does not overproduce clusters, and that the high precision yields cluster sizes broadly consistent with the expected values. However, the observed MUC score of 100.0 may be explained by the large proportion of singleton clusters~\cite{recasens_hovy_2011}.
...
\begin{figure}
    ...
    \caption[Absolute frequency of entity clusters by size vs the expected values.]{The absolute frequency of entity clusters by size (the points) vs the expected values (the curve) from the test-set.}
    \label{fig:clusters}
\end{figure}
...
& (c) Link$_{\text{New}}$ &   n/a* &  36.36 &  76.92 &  68.75 &  73.33 &  70.00 &  70.59 &  85.00 &  69.57 &  61.90 &  61.24 & \\ %error: should be 68.04666666666667 (i divided by 10 instead of 9) TODO fix
...
\bottomrule
\end{tabular}
    \caption[\acs{inel} Evaluation results.]{\acs{inel} Evaluation results: \textit{\ac{nel}}, \textit{NIL Pred}, \textit{Overall}, and \textit{Correct} are obtained using the pipeline with the 3-step clustering algorithm (the top performing one). \textit{Correct} results are obtained correcting the output of the previous components. $\varnothing$ refers to the unbatched single-pass experiment. NIL prediction performance are calculated considering correct when an \ac{nel} error is mitigated. *Nothing can be linked to previously added entities in $b_{0}$. ``--'' represents cases where ground truth clusters contain one element; in this case, \textit{MUC F$_1$} score (= 0) is not meaningful~\cite{recasens_hovy_2011}.}
    \label{tab:superresults}
\end{table}
\end{landscape}

These errors can be mitigated through a \acl{hitl} validation process, where a user, with a dedicated \acp{ui}, can merge clusters referring to the same entity or split those that incorrectly group different mentions. This functionality has been developed as part of this \phd and is illustrated in \Cref{sec:iaui-from-prototypes}.




\subsection{Discussion and Challenges in Incremental Entity Linking}
% hitl


%Results obtained in this work show that the approach is promising, using pretrained neural networks, but can be further improved by the addition of new modules and improvements of existing ones.
...
%A future version may include more features, based on candidates distribution, as well as a standardization layer so that it can be pretrained on a different dataset.
...
% We leave this aspect for future work, as it is tightly connected to the long-tail entity problem (\ie entities mentioned only once). In fact, our dataset was enriched with many such entities.
