...
\begin{figure}[htbp]
    ...
    \caption{Schema of the \acl{inel} pipeline.
    }
    \label{fig:pipeline}
\end{figure}
...
\bottomrule
\end{tabular}
    \caption{NIL prediction feature ablation study on the WNUM~\cite{Onoe_Durrett_2020} dataset, using the training set for model fitting and the development set for evaluating. \textit{max} is the score of the best candidate, \textit{levenshtein} and \textit{jaccard} are textual similarities between the mention and the entity title, \textit{secondiff} is the difference between the best and the second-best score.}
...
\todo{break figures before starting this}
...
\begin{figure}
    ...
    \caption{The absolute frequency of entity clusters by size (the points) vs the expected values (the curve) from the test-set.}
    \label{fig:clusters}
\end{figure}
