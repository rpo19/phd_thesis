...
In this section, we address \ref{ob:extraction_assessment}, especially focusing on the \nilchallenge, by evaluating and adapting existing \ac{nel} techniques to \acf{inel}---which extends \acl{nel} techniques to an incremental scenario where NIL mentions are identified, clustered, and finally added to the reference \ac{kr} (see \Cref{sec:incrementalel} for more).

Indeed, entities in domains such as law or healthcare are often unknown in public \acp{kr}, such as Wikipedia. Examples are the individuals mentiones in court judgments, the suspects of an investigation, or patients in healthcare.
...
This chapter relates to the \phd publication \textcite{ijckg2022}.
