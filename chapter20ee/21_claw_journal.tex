...
These tasks are executed sequentially, thus, each annotation step has been performed by manually refining the output of previous tasks. For this reason, we can consider the annotation process as a \say{simulation} of a real \emph{\acf{hitl}} use case, where humans manually verify and correct automatic annotations, for enhancing their quality.
...
% %, the description of the ground truth (\Cref{sec:gold_standard}),
...
The results of the atomic evaluation for the various knowledge consolidation tasks are reported in \Cref{tab:nereval:atomic}. The \ac{nel} module performs satisfactorily, achieving an accuracy of 73.5\% and a recall@100 of 90.8\%, consistent with results obtained on benchmark datasets.
...
The main weakness lies in the final NIL clustering stage, which obtained low B$^3$ and CEAF$_e$ scores\todo{discuss why muc is 100. singleton clusters?}. Interpreting clustering metrics is inherently difficult, and prior work has shown wide variability across datasets and evaluation settings~\cite{streaming_crossdoc_coreference}. Since suboptimal clustering directly affects the consolidation of NIL mentions, improving this step is crucial. A possible direction is to tune clustering hyperparameters (e.g., thresholds) on domain-specific data, as suggested by recent studies~\cite{streaming_crossdoc_coreference}.\todo{which params we used?}
\todo{ricorda perche succedeva. singleton?}

The NIL prediction component also shows a bias toward the NIL class, which could be reduced through domain-specific training. However, in the context of court judgments, this limitation is less problematic, as most key entities (e.g., plaintiffs, defendants, judges, and attorneys) are in fact NIL.

Performance degradation was also observed in \ac{ner} models trained on publicly available datasets, with the only exception of the \texttt{Person} category. As \ac{ner} is the first component in the pipeline, its errors propagate to subsequent stages, amplifying their impact. This underscores the importance of domain-specific fine-tuning for \ac{ner} models—a topic further explored in our previous work~\cite{aixia_2023} and discussed in the next section (\Cref{sec:aixia}).

Overall, these findings confirm the feasibility of extracting and semantically consolidating entity mentions from Italian court judgments. The proposed architecture also supports \acl{hitl} revision of automatically generated annotations. The \ac{hitl} process used to construct the gold standard allowed us to estimate an average of 30 minutes to fully revise a single judgment, corresponding to approximately 320 judgments per month for one full-time annotator. This estimate provides an upper bound on the cost of human-assisted entity extraction.

% We proved the impact of domain-specific fine-tuning in~\cite{PozziAIxIA23} in which we fine-tuned the SpaCy approach, which utilizes a BERT encoder, on our gold standard, achieving an F$_1$ score of 0.822, with the strictest \textit{strong typed match} criterion. This increase by about 100\% (see \Cref{tab:nereval:bymetric}) has an impact also on the performance evaluated in the \textit{end-to-end} experiment that with the domain-specific \ac{ner} model achieves an F$_1$ of 0.661 with the \textit{strong typed match} criterion (10\% higher of the results calculated with \textit{approximate typed} in \Cref{tab:nereval:withlink}). 
