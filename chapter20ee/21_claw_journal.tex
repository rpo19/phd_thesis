...
In summary, this section makes the following contributions to \acl{ee} applied to Italian civil judgments:
\begin{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{images/ICCJ146-EE_entity_type_distribution.png}
...
$\text{Scott's~}\pi \approx \text{Cohen's~}\kappa \approx \text{Fleiss'~}\kappa \approx \text{Krippendorff's~}\alpha$
 & 66.2           \\\bottomrule
\end{tabular}
\caption[Inter-annotator agreement for \ac{ner} on ICCJ146-EE]{Inter-annotator agreement for \ac{ner} computed on a subset of 30 documents from ICCJ146-EE. The description of each F$_1$ score-based metric can be found in \Cref{sec:metrics}. *based on strong-typed match.}
\label{tab:iaa}
\end{table}
...
We fine-tuned the \spacy pipeline, including both the transformer and the parser, on the Italian WikiNER dataset~\cite{NOTHMAN2013151}, based on Wikipedia articles annotated with \texttt{Person}, \texttt{Location}, \texttt{Organization}, and \texttt{Miscellaneous} entities.
...
\begin{table}[H]
    ...
    \begin{tabular}{l|rrr|rrr}
        ...
        \bottomrule
    \end{tabular}
    \caption[\textit{Atomic} \ac{ner} evaluation on ICCJ146-EE.]{\textit{Atomic} \ac{ner} evaluation on ICCJ146-EE, with different matching criteria (rows), typed and untyped. These results are obtained using the combination of the \ac{ner} algorithms and considering all the evaluated types.}
...
\begin{table}[H]
    ...
    \begin{tabular}{l|l|rrr}
        ...
        \texttt{Organization}  & \spacy   & 58.2 & 40.9 & 48.1\\
                      ...
                      & Trie\ac{ner}    & \textbf{90.0} & 0.2 & 0.5\\ 
    ...
    \end{tabular}
    \caption[\textit{Atomic} \ac{ner} evaluation on ICCJ146-EE by type and algorithm using \textit{approximate-typed match}.]{\textit{Atomic} \ac{ner} evaluation on ICCJ146-EE by type and algorithm using \textit{approximate-typed match}. *\texttt{Miscellanea} mentions are excluded from the overall calculation. **Note that the type Organization is underrepresented in the \acl{kr} used by Trie\ac{ner}, thus only a few entities of this type (\ie $\approx$10) have been predicted.}
...
\begin{table}[H]
    ...
    \begin{tabular}{lr|rrrr}
        ...
        \bottomrule
    \end{tabular}
    \caption{\textit{Atomic} evaluation of the knowledge consolidation tasks and joint evaluation of \ac{nel} and NIL prediction on ICCJ30-\acs{inel}.}
...
\begin{table}[h]
    ...
    \begin{tabular}{l|rrr}
        ...
        \bottomrule
    \end{tabular}
    \caption[\ac{nel} results on benchmark datasets.]{\ac{nel} results on benchmark datasets: strict (s-) and relaxed (r-) version of Italian VoxEL~\cite{rosales2018voxel} and NEEL-IT@Evalita 2016~\cite{basile2016neelit}. for NEEL-IT, Twitter profile tags (@username) and hashtags (\#tag) were filtered out.}
    \label{tab:nelsota}
\end{table}
