...
\begin{itemize}
    ...
    \item precision, recall, and F$_1$ of the NIL prediction,\todo{specifcy for both the classes? also above}
    ...
    \item For the end-to-end \ac{ee}, we proceed similarly as in \Cref{sec:judgment_ee_eval}, using the \textit{strong typed linking} and \textit{partial typed linking} matching criteria. An annotation is correct when the predicted class matches the ground truth, the span matches according to the matchin criterion, and the mention is linked to the correct entity (if $\neg$NIL) or correctly identified as NIL. We calculate \textit{precision}, \textit{recall}, \textit{F$_1$-measure} micro and macro-averaged for each class, exactly as in the \ac{ner} evaluation. While in previous experiments we priviledged the approximate criterion, here we use strong and partial---the strictest and the most relaxed---to estimate lower and upper bounds for the performance achievable with fine-tuning.
\end{itemize}
