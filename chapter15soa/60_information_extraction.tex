...
``\emph{\Acf{ie}} refers to the automatic extraction of structured information such as entities, relationships between entities, and attributes describing entities from unstructured sources''~\cite{infoextraction2008}.
\end{definition}
...
In this work, we focus on \acf{ee} concentrating on entities, without considering broader \ac{ie} tasks such as relation extraction. 

In order, we define and describe the literature of \acl{ner} (\Cref{sec:rw:ner}), \acl{nel} (\Cref{sec:rw:nel}), NIL prediction and clustering (\Cref{sec:incrementalel}), \ac{ee} in the legal domain (\Cref{sec:sota:ee:legal}), and finally we describe \ac{ee} resources for the Italian language (\Cref{sec:sota:ee:ita}).
