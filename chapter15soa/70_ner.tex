...
\ac{ner}, as an \acl{ie} task, contributes to \acl{sta}, \acl{kbp} and downstream tasks such as entity-based search or maintaining \acp{kb} up-to-date~\cite{entityorientedsearch2018balog}. For example, in \ac{sta}, recognizing entities allows to semantically highlight documents to let readers quickly grasp the main topics discussed from the entities mentioned~\cite{swoopes2025interfacedesignsupportlegal,ziwei_abstractexplorer_2025,raymond_skimming_sci_2023}. 
...
For instance, \spacy offers efficient and accurate \ac{ner} models that can be fine-tuned on custom datasets~\cite{honnibal2020spacy}. Its original models use a transition-based architecture~\cite{AliEtAl2023UnintendedMemorization,lample-etal-2016-neural} in which \ac{ner} is framed as a sequence of state transitions over token representations produced by the tok2vec encoder, enriched with sub-word features (\eg prefixes, suffixes, shapes). SpaCy also provides transformer-based variants in which tok2vec is replaced by a pretrained transformer, as in the ``en\_core\_web\_trf'' model\footnote{\url{https://spacy.io/models/en\#en\_core\_web\_trf}}.
